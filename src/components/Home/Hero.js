...
const Hero = () => {
    ...
    // Fetch topics for search suggestions
    const fetchTopics = async () => {
        try {
            const data = await supabaseApi.fetch('topics', 'select=title,file_urls,class_level,subject,category');
            setTopics(data?.map(t => ({
                topic: t.title,
                url: `/notes/${t.class_level}/${t.subject}/${t.category}`,
                isTopic: true
            })) || []);
        } catch (error) {
            console.error("Hero search fetch failed:", error);
        }
    };
